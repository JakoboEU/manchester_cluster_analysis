...
## k = 9
```{r}
insects_k9 = insect_indicator_species(plant_and_insect_clusters$`9`)
```

Generalists
```{r}
insects_k9$sign[is.na(insects_k9$sign$p.value),]
```
...
```
```{r}
write_indicator_species_result = function(ind, filename) {
  write_csv(ind$sign %>% tibble::rownames_to_column('species'), paste('output', filename, sep = '/'))
}
```

```{r}
write_indicator_species_result(insects_k9, 'insect_indicator_species_9.csv')
```
...
## k = 9
```{r}
insects_species_only_k9 = insects_no_bumblebee_generics_indicator_species(plant_and_insect_clusters$`9`)
write_indicator_species_result(insects_species_only_k9, 'insect_no_non_species_indicator_species_9.csv')
```
...
## k = 9
```{r}
plants_k9 = plant_indicator_species(plant_and_insect_clusters$`9`)
```

Generalists
```{r}
plants_k9$sign[is.na(plants_k9$sign$p.value),]
```
...
```{r}
write_indicator_species_result(plants_k9, 'plant_indicator_species_9.csv')
```
...
```{r}
birds_k9 = bird_indicator_species(bird_clusters$`9`)
```
...
```{r}
write_indicator_species_result(birds_k9, 'bird_indicator_species_9.csv')
```
