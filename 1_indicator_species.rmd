...
## Plant and Insect
```{r}
clusters = read_input_csv('insect-plant-clusters.csv') %>%
...
insects_per_site = read_input_csv('insect-presence.csv') %>% 
  ...
  include_sites_with_no_presence_record(clusters$title) %>% 
  arrange(title)
insects_per_site
```
...
insects_k3 = insect_indicator_species(clusters$`3`)
summary(insects_k3)
```
...
## Insects (k = 6)
```{r}
insects_k6 = insect_indicator_species(clusters$`6`)
summary(insects_k6)
```
...
## Insects (k = 8)
```{r}
insects_k8 = insect_indicator_species(clusters$`8`)
summary(insects_k8)
```
...
## Plants (k = 3)
```{r}
plants_k3 = plant_indicator_species(clusters$`3`)
summary(plants_k3)
```
...
## Insects (k = 6)
```{r}
plants_k6 = plant_indicator_species(clusters$`6`)
summary(plants_k6)
```
...
## Insects (k = 8)
```{r}
plants_k8 = plant_indicator_species(clusters$`8`)
summary(plants_k8)
```
